...
async function createOrder(req, res, next) {
  try {
    ...
    for (const it of data.items) {
      ...
      itemsDetailed.push({ productId: it.productId, variantId: it.variantId || null, quantity: it.quantity, name: product.name, sku: product.sku, price });
    }
    ...
    // create checkout
    const checkout = await komerzaService.createCheckout({ orderId: order.id, amount: total, currency: data.currency, metadata: { orderId: order.id } });

    // attach komerza id to order (best-effort)
    await prisma.order.update({ where: { id: order.id }, data: { komerzaId: checkout.komerzaId } });

    res.status(201).json({ order: { id: order.id, total: String(order.total), status: order.status }, checkout });
  } catch (err) { next(err); }
}
